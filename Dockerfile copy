...
RUN --mount=type=secret,id=tokens.env \
    curl -sSL https://raw.githubusercontent.com/idelchi/godyl/refs/heads/dev/install.sh | sh -s -- -d /opt -v v0.0.10 && \
    # Install GoReleaser
    /opt/godyl dump tools --tags goreleaser/goreleaser,cli/cli,idelchi/go-next-tag | /opt/godyl --env-file=/run/secrets/tokens.env i - --output=/usr/local/bin && \
    rm -rf /tmp/godyl
