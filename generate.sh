...
mkdir -p internal/rusti

# Create binary.go
cat << 'EOF' > internal/rusti/binary.go
package rusti
...
# Create installer.go
cat << 'EOF' > internal/rusti/installer.go
package rusti
...
func (i *Installer) Install(package string) (output string, err error) {
	var stdoutBuf, stderrBuf bytes.Buffer

	cargoPath := i.Binary.File.Dir().Join("cargo").String()
	cmd := exec.Command(cargoPath, "install", package)
	cmd.Env = os.Environ()
...
echo "Rust toolchain manager package generated in internal/rusti/"
