...
func (e *Extensions) UnmarshalYAML(node ast.Node) (err error) {
	*e, err = unmarshal.SingleOrSlice[string](node)
	if err != nil {
...
func (e Extensions) ToHint() match.Hint {
	...
	for _, ext := range slices.Compact(e) {
		if ext == "" {
			noExtensionPart = "^[^.]+$"
		} else {
