...
func (ts Tools) ResolveInheritance(d *defaults.Defaults) error {
	for _, t := range ts {
		...
		var hints match.Hints
		// Before merging, check whether the tool already has some hints defined,
		// and if so, clear them out and store them for later appending.
		if t.Hints != nil {
			hints = *t.Hints
			t.Hints = nil
		}
		...
		if err := t.MergeInto(toolDefault); err != nil {
			return fmt.Errorf("merging %q into %v: %w", t.Name, t.Inherit, err)
		}

		// Reapply the original hints
		t.Hints.Append(hints)
	}
