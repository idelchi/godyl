...
func (f File) WriteYAML(v any) error {
	// Marshal the struct to YAML with proper indentation
	const indent = 2
...
// ReadAsString reads the file contents and returns it as a string.
...
// WithSuffix returns a new File with the specified suffix added to the original file name.
func (f File) WithExtension(extension string) File {
	return New(f.WithoutExtension().Path() + "." + extension)
}
